    ...
    if($User->access_control($Form->page_access($page_data['id'])))
    {
        ...
        if(isset($_POST['submit']))
        {
            ...
            $data_list = Array (
                ...
                "read_time"  => $ar_post_clean['read_time'],
                "author_source"  => $ar_post_clean['author_source'],
                "active" => $active,
                "sort"  => $sort,
                                            ...
                                            <div id="item_description">
                                                ...
                                                <div class="t1 tab_content" style="display: block;">
                                                    ...
                                                    <div class="form-group">
                                                        <label class="col-md-12">Categoria <span style="color: red;">*</span></label>
                                                        <div class="col-md-12">
                                                            <select class="form-control required" name="section_id[]" multiple>
                                                                <?php
                                                                $sections = explode(',',$element['section_id']);
                                                                $get_blog_cat = $db
                                                                    ->orderBy('sort','asc')
                                                                    ->get('blog_categories');
                                                                if (count($get_blog_cat) > 0)
                                                                {
                                                                    foreach ($get_blog_cat as $bcat) {
                                                                        ?>
                                                                        <option value="<?php echo $bcat['id']; ?>"
                                                                            <?php if(in_array($bcat['id'], $sections)) { ?> selected <?php } ?>
                                                                        >
                                                                            <?php echo $bcat['title_ro']; ?>
                                                                        </option>
                                                    ...
                                                    $Form->edit_datetimepicker($element['date'],'date', 1);
                                                    $Form->edit_title($element['read_time'], 'read_time', 1,'BLOG_READ_TIME');
                                                    $Form->edit_title($element['author_source'], 'author_source', 1,'BLOG_AUTHOR_SOURCE');

                                                    if(!empty($db_table_dropzone1_images))
                                                    {
                                                        $Form->edit_dropzone_files($db_table_dropzone1_images, $edit_elem_id);
                                                    }
                                                    if(!empty($db_table_dropzone2_images))
                                                    {
                                                        $Form->edit_dropzone_files($db_table_dropzone2_images, $edit_elem_id);
                                                    }
                                                    if(!empty($db_table_dropzone3_images))
                                                    {
                                                        $Form->edit_dropzone_files($db_table_dropzone3_images, $edit_elem_id);
                                                    }
                                                ...
                                                foreach( $list_of_site_langs as $lang_index )
                                                {
                                                    ...
                                                    <div class="t<?php echo $c_tab++?> tab_content">
                                                        ...
                                                        $Form->edit_title($element['page_title_'.$lang_index], 'page_title_'.$lang_index, 1, dictionary('PAGE_TITLE'));
