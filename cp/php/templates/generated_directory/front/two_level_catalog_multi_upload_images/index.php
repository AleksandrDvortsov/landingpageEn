...
<div id="main_block">
    ...
    <?php
    // Deoarece blocul este de tip dropzone, aflam tabelul unde se afla imaginile dropzone.
    $dropzone_table1 = $db
        ->where("id", $page_data['cat_id'])
        ->getOne("pages", "dropzone_image_table1");
            ...
            <a href="<?php echo $Cpu->getURL($detail_page_id, $table_info['id']);?>">
                <div class="image">
                    ...
                    $main_image_of_dropzone = $db
                        ...
                        ->orderBy('sort', 'asc')
                        ->getOne($dropzone_table1['dropzone_image_table1']);

                    $thumb_image = @newthumbs($main_image_of_dropzone['image'], $dropzone_table1['dropzone_image_table1'], 200,150,9,1);
                    if($thumb_image)
                    {
                        ?>
                        <img src = "<?php echo $thumb_image; ?>" alt="<?php echo $main_image_of_dropzone['original_file_name'];?>" title="<?php echo $table_info['title_'.$Main->lang];?>">
                    <?php
                    }
                    ?>
                </div>
